...
# Read API key from api file (check both model/ and parent backend/ directory)
api_key_path = Path(__file__).parent / "api"
if not api_key_path.exists():
    # Try parent directory (backend/api)
...
if api_key_path.exists():
    ...
    if api_key:
        ...
        # Fallback to environment variable if api file is empty
        client = OpenAI()
else:
...
def classify_comment(comment: str) -> str:
    try:
        ...
        if "label" not in parsed:
            error_msg = f"OpenAI response missing 'label' key. Response: {response_content}"
            print(f"ERROR: {error_msg}", file=sys.stderr)
            raise KeyError(error_msg)
        return parsed["label"]
    except json.JSONDecodeError as e:
        error_msg = f"Failed to parse JSON from OpenAI: {str(e)}. Response: {resp.choices[0].message.content if 'resp' in locals() else 'N/A'}"
        print(f"ERROR: {error_msg}", file=sys.stderr)
        raise
    except Exception as e:
        # Better error handling - return a default or re-raise with context
